...
6. **Credit line.** A single small `art: limezu.itch.io` at the bottom, with the `privacy` link and the **app version** sharing the line. The credit is a license requirement (section 4.2); the privacy link is guideline 5.1.1(i); the version is there for the same reason both of the others are, which is that with no about window, no settings screen and no menu bar of its own, the popover is the only place in the app any of it can live. The version is read from the bundle rather than written into the page, so it cannot disagree with what `tools/release.sh` bumped.
